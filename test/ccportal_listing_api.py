...
yesterday = (datetime.now().date() - timedelta(days=3)).strftime("%d/%m/%Y") # Keep 1 Day before date ie day=1
today_date = (datetime.now(pytz.timezone('Asia/Kolkata')) - timedelta(days=2)).strftime("%d-%m-%Y %H:%M:%S") # Keep today's Day before date ie day=0
search_string = (datetime.now() - timedelta(days=3)).strftime("%Y%m%d") # Keep 1 Day before date ie day=1
